...
If the store link will not install for you, or the Gmail connect step keeps failing, reply and tell me. I can send you a test build you load directly into Chrome instead, and we will get you running that way.
...
- The email body follows the house writing rules: plain assertions, no dash punctuation, and
  no contrarian assertion patterns.
- The fallback sentence offers the manual test build (Load-Unpacked from `extension_test/`)
  to anyone the store path fails for. Keep the store URL as the primary route in every send.